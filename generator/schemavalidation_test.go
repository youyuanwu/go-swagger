				...
				if assert.True(t, p.Required) {
					...
					if assert.NoError(t, err) {
						...
						if assert.NoError(t, err) {
							...
							assertInCode(t, k+") Validate(ctx context.Context, formats", res)
			...
			if assertValidation(t, "\"name\"", "m.Name", prop) {
				...
				if assert.NoError(t, err) {
					...
					if assert.NoError(t, err) {
						res := string(formatted)
						assertInCode(t, k+") Validate(ctx context.Context, formats", res)
						assertInCode(t, "m.validateName(ctx, formats", res)
			...
			if assertValidation(t, "\"age\"", "m.Age", prop) {
				...
				if assert.NoError(t, err) {
					...
					if assert.NoError(t, err) {
						res := string(formatted)
						assertInCode(t, k+") Validate(ctx context.Context, formats", res)
						assertInCode(t, "m.validateAge(ctx, formats", res)
			...
			if assertValidation(t, "\"tags\"", "m.Tags", prop) {
				...
				if assert.NoError(t, err) {
					...
					if assert.NoError(t, err) {
						res := string(formatted)
						assertInCode(t, k+") Validate(ctx context.Context, formats", res)
						assertInCode(t, "m.validateTags(ctx, formats", res)
			...
			if assertValidation(t, "", "m", gm.GenSchema) {
				...
				if assert.NoError(t, err) {
					...
					if assert.NoError(t, err) {
						res := string(formatted)
						assertInCode(t, k+") Validate(ctx context.Context, formats", res)
						assertInCode(t, k+"Meta) Validate(ctx context.Context, formats", res)
						assertInCode(t, k+") validateMeta(ctx context.Context, formats", res)
						assertInCode(t, "m.Meta.Validate(ctx, formats)", res)
			...
			if assertValidation(t, "\"args\"", "m.Args", prop) {
				...
				if assert.NoError(t, err) {
					...
					if assert.NoError(t, err) {
						res := string(formatted)
						assertInCode(t, k+") Validate(ctx context.Context, formats", res)
						assertInCode(t, k+"Args) Validate(ctx context.Context, formats", res)
						assertInCode(t, k+"ArgsMeta) Validate(ctx context.Context, formats", res)
						assertInCode(t, "m.validateArgs(ctx, formats", res)
			...
			if assertValidation(t, "", "m", gm.GenSchema) {
				...
				if assert.NoError(t, err) {
					...
					if assert.NoError(t, err) {
						res := string(formatted)
						assertInCode(t, k+") Validate(ctx context.Context, formats", res)
						assertInCode(t, k+") validateP0(ctx context.Context, formats", res)
						assertInCode(t, k+") validateP1(ctx context.Context, formats", res)
			...
			if assertValidation(t, "\"meta\"", "m.Meta", prop) {
				...
				if assert.NoError(t, err) {
					...
					if assert.NoError(t, err) {
						res := string(formatted)
						assertInCode(t, k+") Validate(ctx context.Context, formats", res)
						assertInCode(t, "m.validateMeta(ctx, formats", res)
			...
			if assertValidation(t, "", "m", gm.GenSchema) {
				...
				if assert.NoError(t, err) {
					...
					if assert.NoError(t, err) {
						...
						assertInCode(t, k+") Validate(ctx context.Context, formats", res)
						assertInCode(t, "for k := range m {", res)
						assertInCode(t, "val.Validate(ctx, formats)", res)
						assertInCode(t, "err := validate.MinLength(\"name\",", res)
						assertInCode(t, "err := validate.MaxLength(\"name\",", res)
						assertInCode(t, "err := validate.Pattern(\"name\",", res)
						assertInCode(t, "err := validate.MinimumInt(\"age\",", res)
			...
			if assertValidation(t, "", "m", gm.GenSchema) {
				...
				if assert.NoError(t, err) {
					...
					if assert.NoError(t, err) {
						res := string(formatted)
						assertInCode(t, k+") Validate(ctx context.Context, formats", res)
						assertInCode(t, k+") validateName(ctx context.Context, formats", res)
						assertInCode(t, k+") validateAge(ctx context.Context, formats", res)
						assertInCode(t, k+") validateArgs(ctx context.Context, formats", res)
						assertInCode(t, k+") validateAssoc(ctx context.Context, formats", res)
						assertInCode(t, k+") validateOpts(ctx context.Context, formats", res)
						assertInCode(t, k+") validateExtOpts(ctx context.Context, formats", res)
						assertInCode(t, k+") validateCoords(ctx context.Context, formats", res)
		...
		if assert.NoError(t, err) && assert.Len(t, gm.AllOf, 2) {
			...
			if assert.NoError(t, err) {
				...
				if assert.NoError(t, err) {
					res := string(formatted)
					assertInCode(t, k+") Validate(ctx context.Context, formats", res)
					assertInCode(t, "m.NamedString.Validate(ctx, formats)", res)
					assertInCode(t, "m.NamedNumber.Validate(ctx, formats)", res)
					assertInCode(t, "errors.CompositeValidationError(res...)", res)
				}
			}
			//}
